...
const TabOne = () => {
  ...
  const getApiData = async () => await callEndpoint('https://rickandmortyapi.com/api/character/1');

  const success = (data: any) => setData(data.name);
  const failure = (error: any) => console.log(error);

  useAsync(getApiData, success, failure);
...
const TabTwo = () => {
  ...
  const getApiData = async () => await callEndpoint('https://rickandmortyapi.com/api/character/2');
